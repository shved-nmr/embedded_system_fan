...
void delayMicroseconds(unsigned int us) {
	...
	uint64_t difference = (uint64_t) us * Chip_Clock_GetSystemClockRate()
			/ 1000000;
	// disable RIT – compare value may only be changed when RIT is disabled
	Chip_RIT_Disable(LPC_RITIMER);
	// set compare value to RIT
	Chip_RIT_SetCompareValue(LPC_RITIMER, difference);
	// clear RIT counter (so that counting starts from zero)
	Chip_RIT_SetCounter(LPC_RITIMER, 0);
	// enable RIT
	Chip_RIT_Enable(LPC_RITIMER);
	// wait until RIT Int flag is set (ENABLE RIT IN MAIN WHEN STUCK)
...
LiquidCrystal::LiquidCrystal(DigitalIoPin *rs, DigitalIoPin *enable,
		DigitalIoPin *d0, DigitalIoPin *d1, DigitalIoPin *d2,
		DigitalIoPin *d3) {
	rs_pin = rs;
	enable_pin = enable;
...
void LiquidCrystal::print(std::string const &s) {
	for (int i = 0; i < s.size(); i++) { //for every character in function argument
		write(s[i]);
	}
}
